...
void ModelState::importGLTF(const std::string& path)
{
    tf::TinyGLTF loader;
    tf::Model model;
    std::string warning;
    ...
    bool result = loader.LoadBinaryFromFile(&model, &error, &warning, path);
    ...
    if (result
        && !model.meshes.empty())
    {
        //TODO we want to show some sort of pop-up which allows the user to
        //select one of the contained models to import, or show an error if
        //no models are contained in the scene.
        ...
        //for now let's just assume we want to import the first mesh. Ideally
        //we want to tie this in with ImGui
        ...
        for (const auto& prim : mesh.primitives)
        {
            ...
            for (auto i = 0u; i < tempData.size(); ++i)
            {
                if (attribIndices[i] > -1)
                {
                    ...
                    if (accessor.componentType == GL_FLOAT)
                    {
                        ...
                        CRO_ASSERT(accessor.bufferView < model.bufferViews.size(), "");
                        const auto& view = model.bufferViews[accessor.bufferView];
                        CRO_ASSERT(view.buffer < model.buffers.size(), "");
            ...
            //copy the index data
            CRO_ASSERT(prim.indices < model.accessors.size(), "");
