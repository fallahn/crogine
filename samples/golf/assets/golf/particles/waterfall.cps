particle_system waterfall
{
    texture_smoothing = false
    blendmode = alpha
    acceleration = 3.087000
    gravity = 0.000000,-1.092000,0.000000
    velocity = 0.000000,1.000000,0.000000
    spread = 50.737999
    lifetime = 1.595000
    lifetime_variance = 0.000000
    colour = 255.000000,255.000000,255.000000,255.000000
    random_initial_rotation = true
    rotation_speed = 0.000000
    scale_affector = 0.839000
    size = 0.086000
    emit_rate = 10.000000
    emit_count = 2
    spawn_radius = 0.322000
    spawn_offset = 0.057000,0.000000,0.000000
    release_count = 0
    inherit_rotation = true
    frame_count = 1
    loop_count = 0
    animate = false
    random_frame = false
    framerate = 1.000000

    forces 
    {
        force = 0.000000,0.000000,0.000000
        force = 0.000000,0.000000,0.000000
        force = 0.000000,0.000000,0.000000
        force = 0.000000,0.000000,0.000000

    }
}
