...
void GolfState::updateLensFlare(cro::Entity e, float)
{
    ...
    auto ndc = m_skyScene.getActiveCamera().getComponent<cro::Camera>().getActivePass().viewProjectionMatrix * glm::vec4(m_sunPosition, 1.f);
    ...
    static constexpr std::int32_t FlareCount = 2; //set the initial stride from sun to screen centre - total is more than this
    ...
    if (visible)
    {
        ...
        screenPos /= ndc.w;
        occluded = !ndcVisible(screenPos); //|| (glm::ength2() < glm::length2());

        if (!occluded)
        {
            const glm::vec2 OutputSize(m_gameSceneTexture.getSize() / 2u);
            const glm::vec2 QuadSize(64.f * m_viewScale.x);

            //use length of screenPos to calc brightness / set vert colour
            const float Brightness = ((1.f - std::min(1.f, glm::length2(screenPos))) * 0.2f) + 0.1f;
