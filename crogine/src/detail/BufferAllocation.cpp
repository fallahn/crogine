...
BufferAllocator::BufferAllocator(std::uint32_t blockSize, std::uint32_t vertexSize, std::uint32_t target)
    ...
    m_bufferID              (0),
    m_finalOffset           (0),
    m_bufferAllocationSize  (m_blockSizeBytes * MinBufferSize)
{
    ...
    {
        glCheck(glBindBuffer(m_target, m_bufferID));
        glCheck(glBufferData(m_target, m_bufferAllocationSize, nullptr, GL_DYNAMIC_DRAW));
...
//public
std::size_t BufferAllocator::getBlockCount(std::size_t chunkCount) const
{
    auto blocks = chunkCount / m_blockSize;
    if (chunkCount % m_blockSize)
    {
        blocks++;
    }
    return blocks;
...
BufferAllocation BufferAllocator::newAllocation(std::size_t chunkCount)
{
    const auto blocks = getBlockCount(chunkCount);
    ...
    if (m_finalOffset >= m_bufferAllocationSize)
    {
        //we need to resize the buffer (maintaining the existing data...)
        std::vector<std::uint8_t> oldData(m_bufferAllocationSize);

        glCheck(glBindBuffer(m_target, m_bufferID));
        glCheck(glGetBufferSubData(m_target, 0, m_bufferAllocationSize, oldData.data()));

        do
        {
            m_bufferAllocationSize += MinBufferSize * m_blockSizeBytes;
        } while (m_bufferAllocationSize <= m_finalOffset);

        glCheck(glBufferData(m_target, m_bufferAllocationSize, nullptr, GL_DYNAMIC_DRAW));
        glCheck(glBufferSubData(m_target, 0, oldData.size(), oldData.data()));
        glCheck(glBindBuffer(m_target, 0));
...
void BufferAllocator::freeAllocation(BufferAllocation allocation)
{
