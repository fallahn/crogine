...
#include <crogine/gui/detail/imGuIZMOquat.h>
    ...
    auto drawComponent = [&] (const int idx, const quat &q, ptrFunc func)
    {
        ...
        for(auto itVtx = ptrVtx->begin(), itNorm = (arrowNorm+idx)->begin(); itVtx != ptrVtx->end(); ) { 
            ...
            for(int h=0; h<3; h++) {
                ...
                col[h] = addLightEffect(vec4(directionColor.x, directionColor.y, directionColor.z, 1.f), norm.z, coord.z>0 ? coord.z : coord.z*0.5f);
            }
            addTriangle();
        }
    };
