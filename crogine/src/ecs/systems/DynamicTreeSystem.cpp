...
{
    entity.getComponent<DynamicTreeComponent>().m_treeID = addToTree(entity);
}

void DynamicTreeSystem::onEntityRemoved(Entity entity)
{
    removeFromTree(entity.getComponent<DynamicTreeComponent>().m_treeID);
}
...
//private
std::int32_t DynamicTreeSystem::addToTree(Entity entity)
{
...
bool DynamicTreeSystem::moveNode(std::int32_t treeID, Box worldArea, glm::vec3 displacement)
{
    CRO_ASSERT(treeID > -1 && treeID < m_nodeCapacity, "Invalid tree id");
    CRO_ASSERT(m_nodes[treeID].isLeaf(), "Not a leaf node!");
...
Box DynamicTreeSystem::getFatAABB(std::int32_t treeID) const
{
    CRO_ASSERT(treeID > -1 && treeID < m_nodeCapacity, "Invalid tree id");
