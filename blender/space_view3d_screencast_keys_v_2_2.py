...
# Contributed to by:
# Bart Crouch, John E. Herrenyo, Gaia Clary, Pablo Vazquez, Spivak Vladimir (cwolf3d)
# 
# <pep8 compliant>

bl_info = {
    "name": "Screencast Keys",
    "author": "Paulo Gomes, Vladimir Spivak(cwolf3d)",
    "version": (2, 2),
    "blender": (2, 80, 0),
    "location": "View3D > Tool Shelf > Misc Tab > Screencast Keys",
    "warning": "",
...
def draw_callback_px_text(self, context):
    ...
    if sc.screencast_keys_mouse != 'icon' and sc.screencast_keys_mouse != 'icon+text':
        keypos_x -= mouse_size * MOUSE_RATIO
    if sc.screencast_keys_mouse_position == 'right' and (sc.screencast_keys_mouse != 'icon' or sc.screencast_keys_mouse != 'icon+text'):
        keypos_x = pos_x
    ...
    for i in range(len(self.key)):
        ...
        if label_time < label_time_max: # only display key-presses of last 2 seconds
            ...
            blf.color(0, font_color_r, font_color_g, font_color_b, font_color_alpha * alpha)
    ...
    if sc.screencast_keys_mouse == 'icon' or sc.screencast_keys_mouse == 'icon+text':
        for shape in ["mouse", "left_button", "middle_button", "right_button"]:
            draw_mouse(context, shape, "outline", font_color_alpha)
        final = 0
...
def draw_callback_px_box(self, context):
    ...
    if box_draw and not box_hide:
        ...
        x1 = pos_x + box_width * 1.2 + mouse_size * MOUSE_RATIO + padding_x
        ...
        shader.bind()
        shader.uniform_float("color", (box_color_r, box_color_g, box_color_b, box_color_alpha))
        batch.draw(shader)
...
def draw_last_operator(context, pos_x, pos_y):
    ...
    if wm.operators:
        ...
        blf.position(0, pos_x - 14, pos_y - 50, 0)
        blf.color(0, font_color_r, font_color_g, font_color_b, font_color_alpha)
...
def draw_timer(context, pos_x, pos_y):
    ...
    pos_x = context.region.width - (sc.screencast_keys_timer_size * 12)
    pos_y = 10

    #draw time
    blf.size(0, sc.screencast_keys_timer_size, 72)
    blf.position(0, pos_x, pos_y, 0)
    blf.color(0, timer_color_r, timer_color_g, timer_color_b, timer_color_alpha)
...
def draw_mouse(context, shape, style, alpha):
    ...
    if style == "filled":
        ...
        for data in shape_data:
            x, y, z = data
            coords.append([mouse_size * x + offset_x, mouse_size * y + offset_y])
            indices.append([0, k, k+1])
            k += 1
            
            if (k + 1) <= (len(shape_data)):
                indices.append([0, k, k+1])
                k += 1
...
class ScreencastKeysStatus(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if scene.screencast_keys_mouse == 'icon' or scene.screencast_keys_mouse == 'none':
        ...
        self.last_activity == 'KEYBOARD' and event.type in mouse_keys):
            ...
            if event.type in mouse_keys and \
            ...
            if event.type in mouse_keys and \
            scene.screencast_keys_mouse == 'icon+text':
                #print("Recorded as mouse press")
                self.mouse.insert(0, event.type)
                self.mouse_time.insert(0, time.time())
...
def init_properties():
    ...
    scene.screencast_keys_mouse = bpy.props.EnumProperty(
        items=(("none", "No Mouse", "Don't display mouse events"),
              ...
              ("icon+text", "Icon + Text", "Display graphical representation of "\
               "the mouse and display mouse events as text lines")),
        name="Mouse Display",
...
class OBJECT_PT_keys_status(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if not wm.screencast_keys_keys:
            ...
            row = layout.row(align=True)
            row.enabled = sc.screencast_keys_mouse == 'icon' or sc.screencast_keys_mouse == 'icon+text'
