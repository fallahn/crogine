...
//public
void Image::create(uint32 width, uint32 height, Colour colour, Graphics::Format format)
{
    CRO_ASSERT(width > 0 && height > 0, "Invalid image dimension");
    CRO_ASSERT(format == Graphics::RGB || format == Graphics::RGBA, "Invalid format");

    std::size_t size = (format == Graphics::RGB) ? 3 : 4;
    size *= (width * height);
    ...
    uint32 step = (format == Graphics::RGB) ? 3 : 4;
...
bool Image::loadFromFile(const std::string& path)
{
    ...
    if (img)
    {
        ...
        Graphics::Format format = (img->format->BitsPerPixel == 32) ? Graphics::RGBA : Graphics::RGB;
...
bool Image::loadFromMemory(const uint8* px, uint32 width, uint32 height, Graphics::Format format)
{
    CRO_ASSERT(width > 0 && height > 0, "Invalid image dimension");

    if (format != Graphics::RGB &&
...
{
    CRO_ASSERT(m_format == Graphics::RGBA, "RGBA format required");
...
Graphics::Format Image::getFormat() const
{
    return m_format;
}
